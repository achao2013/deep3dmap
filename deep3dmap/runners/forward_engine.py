...
def single_gpu_test(model, data_loader):
    ...
    for data in data_loader:
        ...
        # refer to https://github.com/open-mmlab/deep3dmap/issues/985
...
def make_recursive_func(func):
    def wrapper(vars,size):
        if isinstance(vars, list):
            return func(vars,size)
        elif isinstance(vars, tuple):
            return tuple([wrapper(x,size) for x in vars])
        elif isinstance(vars, dict):
            return {k: wrapper(v,size) for k, v in vars.items()}
        else:
            return func(vars,size)
    return wrapper

def make_recursive_func2(func):
    def wrapper(vars,size,tmpdir):
        if isinstance(vars, list):
            return func(vars,size,tmpdir)
        elif isinstance(vars, tuple):
            return tuple([wrapper(x,size,tmpdir) for x in vars])
        elif isinstance(vars, dict):
            return {k: wrapper(v,size,tmpdir+k if tmpdir else k) for k, v in vars.items()}
        else:
            return func(vars,size,tmpdir)
    return wrapper
...
def collect_results_cpu(result_part, size, tmpdir=None):
    ...
    print('dump ',osp.join(tmpdir, f'part_{rank}.pkl'))
    ...
    if rank != 0:
        ...
        for i in range(world_size):
            ...
            part_result = deep3dmap.core.utils.load(part_file)
