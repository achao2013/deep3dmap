...
def load_model(model_name = 'Data/BFM.mat'):
    ''' load 3DMM model
    Returns:
...
def get_model_num(model):
    '''
    '''
    nver = model['shapePC'].shape[0]/3
    ntri = model['tri'].shape[1]
    return int(nver), int(ntri)

def get_para_num(model):
    ''' 
    '''
    n_shape_para = model['shapePC'].shape[1]
    n_exp_para = model['expPC'].shape[1]
    return n_shape_para, n_exp_para

def get_kpt_ind(model):
    ''' get 68 keypoints index
    '''
    kpt_ind = np.squeeze(model['kpt_ind']) - 1
...
def get_triangles(model, isFull = True):
    ''' get mesh triangles
    Returns:
        triangels: (3, ntri)
    '''
...
def load_pncc_code(name = 'Data/pncc_code.mat'):
    C = sio.loadmat(name)
