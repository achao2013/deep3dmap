...
import engines
from engines.dist import get_dist_info
from engines.fileio import FileClient, get_file_backend
from engines.fileio import load as load_file
from engines.logging import print_log
from engines.model import BaseTTAModel, is_model_wrapper
from engines.utils import deprecated_function, digit_version, mkdir_or_exist
from engines.utils.dl_utils import load_url

# `ENGINE_HOME` is the highest priority directory to save checkpoints
...
def _get_engines_home():
    engines_home = os.path.expanduser(
        os.getenv(
            ENV_ENGINE_HOME,
            os.path.join(
                os.getenv(ENV_XDG_CACHE_HOME, DEFAULT_CACHE_DIR), 'engines')))

    mkdir_or_exist(engines_home)
    return engines_home
...
def get_torchvision_models():
    ...
    if digit_version(torchvision.__version__) < digit_version('0.13.0a0'):
        ...
        json_path = osp.join(engines.__path__[0], 'hub/torchvision_0.12.json')
        model_urls = engines.load(json_path)
...
def get_external_models():
    engines_home = _get_engines_home()
    default_json_path = osp.join(engines.__path__[0], 'hub/openmmlab.json')
    default_urls = load_file(default_json_path)
    assert isinstance(default_urls, dict)
    external_json_path = osp.join(engines_home, 'open_mmlab.json')
...
def get_mmcls_models():
    mmcls_json_path = osp.join(engines.__path__[0], 'hub/mmcls.json')
...
def get_deprecated_model_names():
    deprecate_json_path = osp.join(engines.__path__[0], 'hub/deprecated.json')
...
def load_from_openmmlab(filename, map_location=None):
    ...
    if model_url.startswith(('http://', 'https://')):
        ...
        filename = osp.join(_get_engines_home(), model_url)
        if not osp.isfile(filename):
            raise FileNotFoundError(f'{filename} can not be found.')
        checkpoint = torch.load(filename, map_location=map_location)
    return checkpoint
...
def save_checkpoint(checkpoint,
    ...
    Args:
        ...
        file_client_args (dict, optional): Arguments to instantiate a
            FileClient. See :class:`engines.fileio.FileClient` for details.
