...
from engines.utils.manager import _accquire_lock, _release_lock
...
class FilterDuplicateWarning(logging.Filter):
    ...
    def __init__(self, name: str = 'engines'):
        super().__init__(name)
        self.seen: set = set()
...
class MMLogger(Logger, ManagerMixin):
    ...
    Args:
        ...
        logger_name (str): ``name`` attribute of ``Logging.Logger`` instance.
            If `logger_name` is not defined, defaults to 'engines'.
...
def _get_world_size():
    ...
    try:
        # requires torch
        from engines.dist import get_world_size
    except ImportError:
        return 1
    else:
...
def _get_rank():
    ...
    try:
        # requires torch
        from engines.dist import get_rank
    except ImportError:
        return 0
    else:
        return get_rank()
