...
from engines.utils import is_seq_of
...
class Registry:
    ...
    https://engines.readthedocs.io/en/latest/advanced_tutorials/registry.html.
    """
    ...
    def infer_scope() -> str:
        ...
        if module is not None:
            ...
            # use "engines" to handle some cases which can not infer the scope
            # like initializing Registry in interactive mode
            scope = 'engines'
            print_log(
                'set scope as "engines" when scope can not be inferred. You '
    ...
    def switch_scope_and_registry(self, scope: Optional[str]) -> Generator:
        ...
        Examples:
            >>> from engines.registry import Registry, DefaultScope, MODELS
            >>> import time
            >>> # External Registry
