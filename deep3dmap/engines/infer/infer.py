...
from engines.config.utils import MODULE2PACKAGE
from engines.dataset import pseudo_collate
from engines.device import get_device
from engines.fileio import (get_file_backend, isdir, join_path,
                             list_dir_or_file, load)
from engines.logging import print_log
from engines.registry import FUNCTIONS, MODELS, VISUALIZERS, DefaultScope
from engines.runner.checkpoint import (_load_checkpoint,
                                        _load_checkpoint_to_model)
from engines.structures import InstanceData
...
class BaseInferencer(metaclass=InferencerMeta):
    ...
    2. Forward the data to the model by :meth:`forward`. ``BaseInferencer``
       assumes the model inherits from :class:`engines.models.BaseModel` and
    ...
    Args:
        model (str, optional): Path to the config file or the model name
            ...
            `weights` saved by Engine which contains the config string.
            Defaults to None.
    ...
    def _get_repo_or_mim_dir(scope):
        ...
        # Since none of Deep3dMap series packages are namespace packages
    ...
    def _init_model(
        ...
        if not cfg:
            ...
            try:
                ...
                # However, the early version of Engine will not save config
