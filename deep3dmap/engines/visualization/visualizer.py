...
from engines.dist import master_only
from engines.logging import print_log
from engines.registry import VISBACKENDS, VISUALIZERS
from engines.structures import BaseDataElement
from engines.utils import ManagerMixin
from engines.visualization.utils import (check_type, check_type_and_length,
                                          color_str2rgb, color_val_matplotlib,
                                          convert_overlay_heatmap,
                                          img_from_canvas, tensor2ndarray,
                                          value2list, wait_continue)
from engines.visualization.vis_backend import BaseVisBackend


@VISUALIZERS.register_module()
class Visualizer(ManagerMixin):
    """Engine provides a Visualizer class that uses the ``Matplotlib``
    ...
    Examples:
        ...
        >>> vis.draw_texts(text='Engine',
        >>>               position=np.array([2, 2]),
        >>>               colors='b')
        >>> vis.draw_texts(text=['Engine','achao2013'],
