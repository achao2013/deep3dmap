...
from deep3dmap.models.modulars.sparse_cnn import SPVCNN
from deep3dmap.core.utils.neucon_utils import apply_log_transform
from .modulars.gru_fusion import GRUFusion
from deep3dmap.core.voxel.back_project import back_project
from deep3dmap.core.voxel.generate_grids import generate_grid
...
class NeuConNet(nn.Module):
    ...
    def __init__(self, model_cfgs):
        ...
        self.n_scales = len(model_cfgs.THRESHOLDS) - 1

        alpha = int(self.model_cfgs.BACKBONE2D.ARC.split('-')[-1])
        ch_in = [80 * alpha + 1, 96 + 40 * alpha + 2 + 1, 48 + 24 * alpha + 2 + 1, 24 + 24 + 2 + 1]
        channels = [96, 48, 24]

        if self.model_cfgs.FUSION.FUSION_ON:
            # GRU Fusion
            self.gru_fusion = GRUFusion(model_cfgs, channels)
        # sparse conv
        self.sp_convs = nn.ModuleList()
        # MLPs that predict tsdf and occupancy.
        self.tsdf_preds = nn.ModuleList()
        self.occ_preds = nn.ModuleList()
        for i in range(len(model_cfgs.THRESHOLDS)):
            self.sp_convs.append(
                SPVCNN(num_classes=1, in_channels=ch_in[i],
                       pres=1,
                       cr=1 / 2 ** i,
                       vres=self.model_cfgs.VOXEL_SIZE * 2 ** (self.n_scales - i),
                       dropout=self.model_cfgs.SPARSEREG.DROPOUT)
            )
    ...
    def forward(self, features, inputs, outputs):
        ...
        loss_dict = {}
        # ----coarse to fine----
        for i in range(self.model_cfgs.N_LAYER):
            interval = 2 ** (self.n_scales - i)
            scale = self.n_scales - i

            if i == 0:
                # ----generate new coords----
                coords = generate_grid(self.model_cfgs.N_VOX, interval)[0]
                up_coords = []
                for b in range(bs):
            ...
            if not self.model_cfgs.FUSION.FUSION_ON:
            ...
            for b in range(bs):
                ...
                coords_batch = coords_batch * self.model_cfgs.VOXEL_SIZE + inputs['vol_origin_partial'][b].float()
            ...
            if self.model_cfgs.FUSION.FUSION_ON:
                ...
                if self.model_cfgs.FUSION.FULL:
            ...
            occupancy = occ.squeeze(1) > self.model_cfgs.THRESHOLDS[i]
