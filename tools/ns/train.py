...
import sys
sys.path.insert(0,"/home/achao/code/3d/deep3dmap/")
from configs.lerf.lerf_config import lerf_method
...
def main(config: TrainerConfig) -> None:
    ...
    print("config.load_config:",config.load_config)
    if config.data:
...
def entrypoint():
    ...
    method_configs["lerf"]=lerf_method.config
    descriptions = {"lerf":lerf_method.description}
    AnnotatedBaseConfigUnion = tyro.conf.SuppressFixed[  # Don't show unparseable (fixed) arguments in helptext.
        tyro.conf.FlagConversionOff[
            tyro.extras.subcommand_type_from_defaults(defaults=method_configs, descriptions=descriptions)
        ]
    ]
    #AnnotatedBaseConfigUnion = tyro.extras.subcommand_type_from_defaults(defaults=method_configs, descriptions=descriptions)
    #print('AnnotatedBaseConfigUnion:',AnnotatedBaseConfigUnion)
    main(
        tyro.cli(
