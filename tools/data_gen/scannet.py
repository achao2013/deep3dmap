...
import time
from deep3dmap.core.tsdf.tsdf_volume import TSDFVolume,get_view_frustum,meshwrite
import pickle
import argparse
from tqdm import tqdm
import ray
import torch.multiprocessing
from deep3dmap.datasets.scannet import ScanNetSceneDataset,collate_fn
import numpy as np
...
def process_with_single_worker(args, scannet_files):
    for scene in tqdm(scannet_files):
        ...
        if args.dataset == 'scannet':
            ...
            dataset = ScanNetSceneDataset(n_imgs, scene, args.data_path, args.max_depth)
...
def generate_pkl(args):
    ...
    if not args.test:
        splits = ['train_debug', 'val_debug']
    else:
        splits = ['test']
    for split in splits:
        fragments = []
        with open(os.path.join(args.data_path,'..','output', 'splits', 'scannetv2_{}.txt'.format(split))) as f:
...
if __name__ == "__main__":
    ...
    if args.dataset == 'scannet':
        if not args.test:
            fpath_train=os.path.join(args.data_path, 'output', 'splits','scannetv2_train_debug.txt')
            train_lines=open(fpath_train).readlines()
            files_train=[f.strip() for f in train_lines]
            fpath_val=os.path.join(args.data_path, 'output', 'splits','scannetv2_val_debug.txt')
            val_lines=open(fpath_val).readlines()
            files_val=[f.strip() for f in val_lines]
            files=files_train+files_val
            args.data_path = os.path.join(args.data_path, 'scans')
        else:
            fpath_test=os.path.join(args.data_path, 'output', 'splits','scannetv2_test.txt')
            test_lines=open(fpath_test).readlines()
            files=[f.strip() for f in test_lines]
